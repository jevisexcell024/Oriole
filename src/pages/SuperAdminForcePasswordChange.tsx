...
/** Mirrors src/pages/ForcePasswordChange.tsx exactly — same shape, same
 *  validation, posting to the Super Admin's own one-time endpoint instead.
 *  Closes the bootstrap gap: a printed one-time password that's never forced
 *  to rotate is a real gap, not a detail to defer. */
export function SuperAdminForcePasswordChange() {
  const t = useT();
  const { superAdmin, refresh, logout } = useSuperAdminAuth();
  ...
  useEffect(() => {
    if (superAdmin && !superAdmin.mustChangePassword) navigate("/super-admin/dashboard", { replace: true });
  }, [superAdmin, navigate]);
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      await api.post("/super-admin/auth/complete-password-setup", { current, password: next });
      await refresh();
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0E0E0D] p-6 shadow-xl sm:p-8">
        ...
        <h1 className="mt-4 text-center text-lg font-bold text-white">{t("sad.fpcTitle")}</h1>
        <p className="mt-1.5 text-center text-sm text-[#AEBAC2]">{t("sad.fpcBody")}</p>

        <div className="mt-6 space-y-3">
          <Field label={t("fpc.currentPassword")}>
            <input type="password" className="input h-11" value={current} onChange={(e) => setCurrent(e.target.value)} autoComplete="current-password" placeholder={t("sad.bootstrapPasswordHint")} />
          </Field>
        ...
        <button onClick={() => { void logout(); navigate("/super-admin/login"); }} className="mt-3 flex w-full items-center justify-center gap-1.5 text-xs font-medium text-[#AEBAC2] hover:text-white">
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <label className="block"><span className="mb-1 block text-sm font-medium text-white">{label}</span>{children}</label>;
}
